...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (provider_id === user_id) {
      throw new _AppError.default("You can't create an appoitment with yourself");
    }

    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default("You can only schedule between 8am and 17pm");
    }

    const checkSchedule = await this.appointmentsRepository.findByDate(appointmentDate, provider_id);

    if (checkSchedule) {
      throw new _AppError.default('This appointment is booked');
    }
