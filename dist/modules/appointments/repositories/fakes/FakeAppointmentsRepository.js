...
class AppointmentsRepository {
  ...
  async findByDate(date, provider_id) {
    const findAppointment = this.appointmentRepository.find(appointment => (0, _dateFns.isEqual)(appointment.date, date) && appointment.provider_id === provider_id);
  ...
  async findAllInMonthFromProvider({
    ...
    const appointments = await this.appointmentRepository.filter(appointment => {
