...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (!user) {
      throw new _AppError.default('User not found with provided id');
    }
    ...
    if (password && !old_password) {
      throw new _AppError.default('Must provide old passowrd.');
    }
    ...
    if (password && old_password) {
      ...
      if (!checkOldPassword) {
        throw new _AppError.default('Incorrect old password.');
      }
